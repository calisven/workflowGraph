...
export class DagreNodesOnlyLayout implements Layout {
  ...
  public updateEdge(graph: Graph, edge: Edge): Graph {
    const sourceNode = (graph.nodes.find(n => n.id === edge.source)) as any;
    const targetNode = (graph.nodes.find(n => n.id === edge.target)) as any;
  ...
  public createDagreGraph(graph: Graph): any {
    ...
    this.dagreNodes = graph.nodes.map(n => {
      ...
      node.width = (n as any).dimension.width;
      node.height = (n as any).dimension.height;
      node.x = (n as any).position.x;
      node.y = (n as any).position.y;
      return node;
    });
